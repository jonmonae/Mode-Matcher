...
def st_open_adjusted_setup(lenses, setups_df, s0, lam0, adjustments, sF, rF=10**10, r0=10**10):
    ...
    if type(lenses) == tuple:
        ...
        st.write(f"You have chosen to adjust dual lens setup with a {lens1} m lens first, followed by a {lens2} m lens")
    else:
        lens1 = lenses
        lens2 = 'single'
    ...
    if not single:
        ...
        d0, d1, d2 = setup
        d0_new, d1_new, d2_new = (np.array(setup) + np.array(adjustments)).tolist()
        
        
        st.write(f"For this setup...")
        st.write(f"the adjusted d0 distance: {d0_new} m")
        st.write(f"the adjusted d1 distance: {d1_new} m")
        st.write(f"the d2 distance: {d2_new} m")
        
        
        w, s, zpos = two_lens_out(d0, lens1, d1, lens2, d2, s0, lam0, r0=r0)
        r = radius(zpos, w, lam0)
        
        w_new, s_new, zpos_new = two_lens_out(d0_new, lens1, d1_new, lens2, d2_new, s0, lam0, r0=r0)
        r_new = radius(zpos_new, w_new, lam0)
        
        
        plt = plot_dual(d0_new, lens1, d1_new, lens2, d2_new, s0, lam0, r0=r0)
        st.pyplot(plt)
        ...
        d0, d1 = setup
        d0_new, d1_new = (np.array(setup) + np.array(adjustments)).tolist()
        ...
        w_new, s_new, zpos_new = one_lens_out(d0_new, lens1, d1_new, s0, lam0)
        r_new = radius(zpos_new, w_new, lam0)
        
        
        plt = plot_single(d0_new, lens1, d1_new, s0, lam0, r0=r0)
        st.pyplot(plt)
    ...
    q_targ = q_factor_sr(sF, rF, lam0)
    q_adj = q_factor_sr(s_new, r_new,lam0)
    mode = mode_overlap(q_targ, q_adj)

    
    
    st.write(f"\n\nThe mode overlap for the adjusted setup is:  {mode}")
    st.write(f"The spot size from this setup is: {format_float(s_new,4)} m")
    st.write(f"The radius from this setup is: {format_float(r_new,4)} m")
